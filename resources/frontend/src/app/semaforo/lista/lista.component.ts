...
export class ListaComponent implements OnInit {
  ...
  constructor(
    ...
    public SemaforoService: SemaforoService,
  ...
  public cargarLista(event?: PageEvent) {
    ...
    this.SemaforoService.obtenerLista(params).subscribe(
  ...
  //   dialogRef.afterClosed().subscribe(valid => {
  //     if (valid.estatus) {
  //       this.SemaforoService.actualizarEstatus(valid.id, valid.resultado).subscribe(
  //         response => {
  //           if (response.error) {
  //             let errorMessage = response.error.message;
  //             this.sharedService.showSnackBar(errorMessage, null, 3000);
  ...
  confirmAlta(id: string = '') {
    ...
    dialogRef.afterClosed().subscribe(reponse => {
      if (reponse) {
        this.SemaforoService.registroSalida(id, 2).subscribe(
          response => {
            if (response.error) {
              let errorMessage = response.error.message;
              this.sharedService.showSnackBar(errorMessage, null, 3000);
  ...
  confirmDefuncion(id: string = '') {
    ...
    dialogRef.afterClosed().subscribe(reponse => {
      if (reponse) {
        this.SemaforoService.registroSalida(id, 3).subscribe(
          response => {
            if (response.error) {
              let errorMessage = response.error.message;
              this.sharedService.showSnackBar(errorMessage, null, 3000);
